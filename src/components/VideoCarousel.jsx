...
const VideoCarousel = () => {
  ...
  // Reset all progress bars when video resets
  useEffect(() => {
    if (videoId === 0 && !isLastVideo) {
      // Reset all progress bars
      videoSpanRef.current.forEach((span, index) => {
        if (span && videoDivRef.current[index]) {
          gsap.set(span, {
            width: "0%",
            backgroundColor: "white",
          });
    ...
    <>
      <div className="flex items-center gap-8 sm:gap-12 md:gap-16">
        {hightlightsSlides.map((list, i) => (
          <div key={list.id} id="slider" className="flex-shrink-0">
      ...
      <div className="relative flex justify-center items-center mt-10">
        <div style={{padding: "7px 5px"}} className="flex justify-center items-center bg-[#42424578] backdrop-blur rounded-full">
          {videoRef.current.map((_, i) => (
            <span
              key={i}
              className="mx-2 w-3 h-3 bg-[#afafaf] rounded-full relative cursor-pointer overflow-hidden"
        ...
        <button 
         style={{padding: "10px 7px"}}
          className="ml-4 rounded-full bg-[#42424578] backdrop-blur flex justify-center items-center"
          onClick={
            isLastVideo
